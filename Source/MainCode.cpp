...
namespace
{
	...
	const char* const WINDOW_TITLE = "Peck - 7-1 FinalProject and Milestones";
...
{
	//Creating a filesystem path to use custom glsl files
	namespace fs = std::filesystem;

	//Create variables for preferred and fallback paths for glsl files
	std::string preferredVert = "Utilities/shaders/vertexShader.glsl";
	std::string preferredFrag = "Utilities/shaders/fragmentShader.glsl";

	std::string fallbackVert = "../../Utilities/shaders/vertexShader.glsl";
	std::string fallbackFrag = "../../Utilities/shaders/fragmentShader.glsl";
	...
	std::string finalVert = fs::exists(preferredVert) ? preferredVert : fallbackVert;
	std::string finalFrag = fs::exists(preferredFrag) ? preferredFrag : fallbackFrag;

	g_ShaderManager->LoadShaders(finalVert.c_str(), finalFrag.c_str());
	g_ShaderManager->use();

	// try to create a new scene manager object and prepare the 3D scene
	g_SceneManager = new SceneManager(g_ShaderManager);
	g_SceneManager->SetViewManager(g_ViewManager);

	//Pass the window variable to the scene manager class
