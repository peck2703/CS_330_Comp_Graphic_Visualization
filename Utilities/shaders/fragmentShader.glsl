...
uniform bool bUseSecondaryTexture=false;
uniform vec4 objectColor = vec4(1.0f);

uniform sampler2D objectTexture;
uniform sampler2D secondaryTexture;
uniform float blendAmount = 0.25f;
...
void main()
{
    vec4 baseResult = objectColor;

    if(bUseTexture)
    {
        //Apply tiling UVScale
        vec2 tiledUV = fragmentTextureCoordinate * UVScale;
        baseResult = texture(objectTexture, tiledUV);
    ...
    {
        vec2 tiledUV = fragmentTextureCoordinate * UVScale;
        vec4 tex2 = texture(secondaryTexture, tiledUV);

        //Mix the results
        baseResult = mix(baseResult, tex2, blendAmount);
    }

    vec4 finalTextureColor = baseResult;
    ...
    if(bUseLighting == true)
    {
        ...
        // Apply lighting to the final blended result
        outFragmentColor = vec4(phongResult * finalTextureColor.rgb, finalTextureColor.a);
    }
    else 
    {
        outFragmentColor = finalTextureColor;
    }
}
